...
/**
 * Circular orbit pattern. Math in UTM meters, output in lat/lng.
 * X_m = radius * cos(ω * t), Y_m = radius * sin(ω * t)
 */
public class OrbitMovement implements MovementEngine {
    ...
    public OrbitMovement(MovementConfig config) {
        ...
        if (radiusM <= 0) {
            throw new IllegalArgumentException("radiusM must be positive for ORBIT pattern, got: " + radiusM);
        }
    ...
    public double[] getPosition(double elapsedSeconds) {
        ...
        double metersEast = radiusM * Math.cos(angle);
        double metersNorth = radiusM * Math.sin(angle);
    ...
    public double[] getVelocity(double elapsedSeconds) {
        ...
        double angle = angularVelocity * elapsedSeconds;
        double eastRate = -radiusM * angularVelocity * Math.sin(angle);
        double northRate = radiusM * angularVelocity * Math.cos(angle);
